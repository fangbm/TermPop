...
TermPop 支持 Mock、OpenAI、Kimi、OpenAI-compatible 和 Anthropic 风格服务商。可在弹窗中填写服务商、API Key、模型和 Base URL；“测试连接”会复用扩展已有的访问设置，不额外申请服务商域名权限。
...
Rust 匹配内核支持基础、领域和用户三类词典；用户词典优先于通用规则。扩展将术语缓存和释义缓存分开，以减少重复工作：
...
- 本地 OCR 只在本地识别文字，释义仍需要配置释义服务商或使用 mock fallback。
